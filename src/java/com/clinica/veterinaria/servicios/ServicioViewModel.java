...
public class ServicioViewModel {
    ...
    public ListModelList<Servicio> getEvents() {
        if(events == null) events = new ListModelList<Servicio>();//new ListModelList<Servicio>(eventDao.findAll());
        //events = new ListModelList<Servicio>(eventDao.findAll());
...
//    }
    ...
    @GlobalCommand
    @Command("add")
    @NotifyChange({"events", "selectedEvent", "servicios", "productos"})
    public void add(@BindingParam("cmp")  final Window w) {
        ...
        if(ServicioValidator.validar(newEvent)){
            BindUtils.postGlobalCommand(null, null, "doSearch", null);
            Messagebox.show("Se ha guardado el Servicio", "Mensaje", Messagebox.OK, Messagebox.QUESTION,
    ...
    @Command("update")
    @NotifyChange({"events", "selectedEvent", "servicios", "productos"})
    public void update(@BindingParam("cmp")  final Window w) {
        if(this.selectedEvent.getIva() == null){
            this.selectedEvent.setIva(auxIva);
        }
            eventDao.update(this.selectedEvent);
            
        if(ServicioValidator.validar(selectedEvent)){                 
            Messagebox.show("Se ha actualizado el Servicio " + selectedEvent.getNombre(), "Mensaje", Messagebox.OK, Messagebox.QUESTION,
                new org.zkoss.zk.ui.event.EventListener(){
                    public void onEvent(Event e){
                        if(Messagebox.ON_OK.equals(e.getName())){
                            w.detach();
    ...
    public void delete() {
            ...
            if(this.selectedEvent != null) {
                Messagebox.show("¿Desea elimimar " + selectedEvent.getNombre() + " ?", "Mensaje", Messagebox.OK, Messagebox.QUESTION,
                new org.zkoss.zk.ui.event.EventListener(){
                    public void onEvent(Event e){
                        if(Messagebox.ON_OK.equals(e.getName())){
    ...
    @GlobalCommand
    @Command 
    @NotifyChange({"productos","events"})
    public void doSearch() 
    {
        events = new ListModelList<Servicio>();
