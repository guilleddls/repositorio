...
public class FacturaViewModel {
    ...
    private final FacturaDAO eventDao = new FacturaDAO();
    ...
    public List<Factura> getEvents() {
        if(events == null) {
            events = new ListModelList<>();//new ListModelList<Factura>(eventDao.findAll());
        }
        return events;
    }
    ...
    public ListModelList<Factura> getFacturas() {
        if(facturas == null) {
            facturas = new ListModelList<>();//new ListModelList<Factura>(eventDao.findAll());
        }
        return facturas;
    }
    ...
    public void delete() {
        if(this.selectedEvent != null) {
            Messagebox.show("¿Desea elimimar la Factura Nº " + selectedEvent.getNumero() + " ?", "Advertencia", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION,
             ...
             Messagebox.show("Seleccione una factura", "Mensaje", Messagebox.OK, Messagebox.EXCLAMATION);
    }
    ...
    public void showFacturaLinea(){
        ...
        Executions.createComponents("facturas/factura.zul", null, map);
    }
    ...
    public void doSearch()
    {
        ...
        if((filterNumero   == null || "".equals(filterNumero))    &&
           ...
           (filterEmpleado == null || "".equals(filterEmpleado))) {
                facturas.addAll(eventDao.findAll());
        }
        else
        {
            allEvents = eventDao.findAll();
            for (Factura clie : allEvents) {
