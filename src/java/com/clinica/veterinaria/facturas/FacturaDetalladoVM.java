...
public class FacturaDetalladoVM {
    ...
    private List<VentaLinea> ventalineas = new ArrayList<>();
    private final VentaLineaDAO eventDao = new VentaLineaDAO();
    private final VentaDAO ventaDao = new VentaDAO();
    private final VentaLineaDAO ventalinDao = new VentaLineaDAO();
    ...
    public void delete() {
        ...
        if(this.selectedEvent != null) {
            int id = (ventalinDao.find_idVenta(this.selectedEvent)); //findOne(this.selectedEvent.getId())).getVenta().getId();
            
            List<Venta> list = new VentaDAO().findAll("select * from zk_venta where id=" + id);
            if(list.size() == 1) {
                Venta venta = list.get(0);
                venta.setFactura(null);
                venta.setDeudor(false);
    ...
    public void refreshVentas(@BindingParam("returnVenta") Set<Venta> ventas)
    {
        System.out.println("Tamaño items: " + ventas.size());
        Iterator <Venta> vta = ventas.iterator();
        while(vta.hasNext()){
            Venta ventaAux = vta.next();
            ventaAux.setDeudor(true);
            ventaAux.setFactura(selectedFactura);
            this.selectedFactura.asignarVenta(ventaAux);
            
            ventaDao.updateFactura(ventaAux);
            
            Iterator <VentaLinea> linea = ventaAux.getVenta_lineas().iterator();
            while(linea.hasNext()){
